    ...
    <div class="request-content">
      <div class="request-table-container">
        ...
        <table class="request-table">
          ...
          <tbody>
            <?php if(!empty($data['caregiverPayments'])): ?>
              <?php foreach($data['caregiverPayments'] as $payment): ?>
                <tr>
                  <td>#CG<?php echo $payment->care_request_id; ?></td>
                  <td><?php echo $payment->caregiver_name; ?></td>
                  ...
                  <td><?php echo number_format($payment->caregiver_payment, 2); ?></td>
                  ...
                  <td>
                    <?php if($payment->is_paid == 0 || $payment->is_paid === null): ?>
                        ...
                        <form action="<?php echo URLROOT; ?>/moderator/markCaregiversAsPaid" method="POST">
                        <input type="hidden" name="care_request_id" value="<?php echo $payment->care_request_id; ?>">
                        <input type="hidden" name="caregiver_id" value="<?php echo $payment->caregiver_id; ?>">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  searchInput.addEventListener('keyup', function() {
    ...
    tableRows.forEach(row => {
      ...
      // Get the caregiver name (2nd column), bank name (3rd column), and account holder (5th column)
      const caregiverName = row.querySelector('td:nth-child(2)').textContent.toLowerCase();
